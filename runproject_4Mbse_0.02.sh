...
#SEVNPATH
#-------------------------------
SEVN="/home/maglione/SEVN" #Complete path to the SEVN folder
SEVNEXE="${SEVN}/build/exe/sevnB.x" #Complete path to SEVN BSE executable

#-------------------------------
#RUN OPTIONS
#-------------------------------
NTHREADS="20" #Number of OpenMP threads (1 means no parallel threads, sequential execution)
NCHUNK="100000" #Evolve Nchunk at time
...
#-------------------------------
if [ "$cmdargument" = "" ]; then
  LISTBIN="${SEVN}/imf23_2_paper.in" #Complete path to input file (list of binaries or single stars)
else
  LISTBIN=$cmdargument
fi
...
#METALLICITY
#-------------------------------
Z="0.01"  #Stellar metallicity - [*list][number].
...
SUPERNOVA="rapid" #Prescription for the SN explosion mechanism - [list*] [rapid] [delayed] [compactness]
#---------OPTIONS---------------
