...
dtt=pd.read_csv("/home/mariasole/Documents/comput/SEVNproject/SEVN_bse_0.02/sevn_output_bse_0.02/output_0_bse_0.02.csv")
print(dtt)
print(dtt.columns)

### defyining time ranges in Myr within which plotting the HR diagrams###

tranges=((0,1),(1,2),(2,3),(3,3.5)) #Myr
...
### printing the BEvent of the BS ###
    ...
    print("The number of Roche-Lobe Overflow starting events from", tlow, "<Age<", tup, "is: ",len(RLO_num), "\n", "The number of Roche-Lobe Overflow + Merger events from", tlow, "<Age<", tup, "is: ",len(RLO_merge_num), "\n", "The number of Roche-Lobe Overflow + Common Envelope events from", tlow, "<Age<", tup, "is: ",len(RLO_ce_num), "\n", "The number of Collision + Merger events from", tlow, "<Age<", tup, "is: ",len(coll_merge_num), "\n", "The number of Collision + Common Envelope events from", tlow, "<Age<", tup, "is: ",len(coll_ce_num))
...
### plotting the HR diagram with hexbin - tried also with scatter ###
    ...
    plt.hexbin(np.log10(Temperature),np.log10(Luminosity),cmap="jet",mincnt=1)
    ...
    plt.xlim(5.5,3.5)
    plt.ylim(1.,7.)
    plt.xlabel("$\log T/\mathrm{K}$",fontsize=14)
    plt.ylabel("$\log L/\mathrm{L_\odot}$",fontsize=14)
    plt.gca().tick_params(axis='both', which='major', labelsize=14)
    plt.title(f"{tlow}<Age/Myr<{tup}",fontsize=16)
